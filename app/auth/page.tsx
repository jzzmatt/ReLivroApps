...
export default function AuthPage(){
  ...
  return <main className="auth-page"><div className="auth-card"><div className="auth-language"><LanguageSwitcher/></div><Link className="brand" href="/"><span className="logo-book"><span/></span><span><b>Re</b>Livro<span>Apps</span></span></Link><span className="eyebrow">{mode==="signin"?t.auth.welcome:t.auth.signupEyebrow}</span><h1>{mode==="signin"?t.auth.signinTitle:t.auth.signupTitle}</h1><p>{mode==="signin"?t.auth.signinText:t.auth.signupText}</p><form onSubmit={submit}>{mode==="signup"&&<label>{t.auth.name}<input value={name} onChange={e=>setName(e.target.value)} required placeholder="O seu nome"/></label>}<label>{t.auth.email}<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="nome@email.com"/></label><label>{t.auth.password}<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={6} placeholder="••••••••"/></label><button className="button" disabled={loading}>{loading?t.auth.wait:mode==="signin"?t.auth.signin:t.auth.signup}</button></form>{message&&<div className="auth-message">{message}</div>}<button className="auth-switch" onClick={()=>{setMode(mode==="signin"?"signup":"signin");setMessage("")}}>{mode==="signin"?"Ainda não tenho conta":"Já tenho uma conta"}</button></div></main>
}